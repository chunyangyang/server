...
function selectProvince(type, con, isremove) {
    ...
    if (type == "all") {
        ...
        var guowai   = __LocalDataCities.category.guowai;
        // 加载热门城市和省份
        ...
        strVarCity += '</div>';
        // 港澳台国外
        strVarCity += '    <p class="data-title">国外</p>';
        strVarCity += '   <div class="data-list data-list-provinces">';
        strVarCity += '  <ul class="clearfix">';
        for (var i in guowai) {
            strVarCity += '<li><a href="javascript:;" data-code="' + guowai[i] + '" data-name="' + dataarrary[guowai[i]][0] + '" class="d-item"  onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[guowai[i]][0] + '<label>0</label></a></li>';
        }
        strVarCity += ' </ul>';
        strVarCity += '</div>';
