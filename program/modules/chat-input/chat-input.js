let _page,
 hidden,
 inputObj = {}, recorderManager,
 windowHeight, windowWidth,
// let voice$position = {toLeft: 0, toBottom: 0};
 singleVoiceTimeCount = 0,
 maxVoiceTime = 60, minVoiceTime = 1, startTimeDown = 54,
 timer,
 sendVoiceCbOk, sendVoiceCbError, startVoiceRecordCbOk, tabbarHeigth = 0, extraButtonClickEvent, canUsePress = false,
    voiceFormat,
...
const clickExtraItemListener = cb => {
    _page.chatInputExtraItemClickEvent = typeof cb === "function" ? cb : null;
}

const sendVoiceListener = (cbOk, cbError) => {
    sendVoiceCbError = cbError;
    sendVoiceCbOk = cbOk;
    if (!!recorderManager) {
        typeof cbOk === "function" && (recorderManager.onStop(res => {
        ...
        typeof cbError === "function" && (recorderManager.onError(res => {
...
const setVoiceRecordStatusListener = _ => {
    startVoiceRecordCbOk = arguments[0];
}

const initChangeInputWayEvent = _ => {
    _page.changeInputWayEvent = _ => {
        _page.setData({
...
const setExtraButtonClickListener = fun => {
    extraButtonClickEvent = fun;
}



const initExtraData = extra$arr => {
    _page.setData({ 'inputObj.extraObj.chatInputExtraArr': extra$arr });
    _page.chatInputExtraClickEvent = _ => {
        _page.setData({ 'inputObj.extraObj.chatInputShowExtra': !_page.data.inputObj.extraObj.chatInputShowExtra });
        extraButtonClickEvent && extraButtonClickEvent(!_page.data.inputObj.extraObj.chatInputShowExtra);
      if (_page.data.inputObj.extraObj.chatInputShowExtra==true){
          _page.setData({
            'position':'relative',
            'bottom':'192'
          })
...
const dealVoiceLongClickEventWithHighVersion = _ => {
    recorderManager.onStart( _ => {
        ...
        timer = setInterval( _ => {
            ...
            if (singleVoiceTimeCount >= startTimeDown && singleVoiceTimeCount < maxVoiceTime) {
                ...
                _page.setData({ 'inputObj.voiceObj.status': 'timeout' });
                                    ...
                                    wx.openSetting({
                                        success: res => {
                                            if (res.authSetting['scope.record']) {
                                                _page.setData({ 'inputObj.extraObj.chatInputShowExtra': false });
                                            }
                                        }
                                    });
                                } else {
                                    _page.setData({ 'inputObj.inputStatus' : 'text', 'inputObj.extraObj.chatInputShowExtra' : false });
                                }
                            }
                        });
                    } else {
...
const dealVoiceLongClickEventWithLowVersion = _ => {
    _page.long$click$voice$btn = e => {
        if ('send$voice$btn' === e.currentTarget.id) {//长按时需要打开录音功能，开始录音
            singleVoiceTimeCount = 0;
            _page.setData({//调出取消弹窗
...
const dealVoiceMoveEvent = _ => {
    _page.send$voice$move$event = e => {
        if ('send$voice$btn' === e.currentTarget.id) {
            ...
            if (y > cancelLineYPosition) {
                if (!inputObj.voiceObj.moveToCancel) {
                    _page.setData({ 'inputObj.voiceObj.moveToCancel': true });
                }
            } else {
                if (inputObj.voiceObj.moveToCancel) {//如果移出了该区域
                    _page.setData({ 'inputObj.voiceObj.moveToCancel': false });
                }
            }
...
const dealVoiceMoveEndEvent = _ => {
    _page.send$voice$move$end$event = e => {
        if ('send$voice$btn' === e.currentTarget.id) {
            if (singleVoiceTimeCount < minVoiceTime) {//语音时间太短
                _page.setData({ 'inputObj.voiceObj.status': 'short' });
                delayDismissCancelView();
            } else {//语音时间正常
                _page.setData({ 'inputObj.voiceObj.showCancelSendVoicePart': false, 'inputObj.voiceObj.status': 'end' });
            }
            //关闭定时器
...
const checkRecordAuth = (cbOk, cbError) => {
...
const closeExtraView = _ => {
    _page.setData({ 'inputObj.extraObj.chatInputShowExtra': false });
}

const delayDismissCancelView = _ => {
    setTimeout( _ => {
...
const initData = opt => {
    _page.data.inputObj = inputObj = {
        voiceObj: {},
        inputStyle: {
            sendButtonBgColor: opt.sendButtonBgColor || 'mediumseagreen', sendButtonTextColor: opt.sendButtonTextColor || 'white'
        }
    };
}
...
const setTextMessageListener = cb => {
    if (_page) {
        _page.chatInputBindFocusEvent = _ => {
            _page.setData({
                'inputObj.inputType': 'text'

            })
        };
        _page.chatInputBindBlurEvent = _ => {
            _page.setData({
                'inputObj.inputType': 'none',
                'inputObj.extraObj.chatInputShowExtra': false
            });
        };
        _page.chatInputSendTextMessage = e => {
            _page.setData({
                textMessage: ''
            });
            typeof cb === "function" && cb(e);
        };
        _page.chatInputSendTextMessage02 = _ => {
        ...
        _page.chatInputGetValueEvent = e => {
